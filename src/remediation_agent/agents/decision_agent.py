...
class DecisionAgent:
    ...
    def __init__(self, model_name: Optional[str] = None, temperature: Optional[float] = None) -> None:
        ...
        self._system_message = (
            ...
            "Always reply with a single JSON object (no markdown or code fences) containing the keys: "
            "remediation_type (automatic | human_in_loop | manual_only), confidence_score (0-1 number), "
            "reasoning (string), estimated_effort (integer minutes), risk_if_delayed (low | medium | high | critical), "
            "prerequisites (array of strings), and recommended_actions (array of strings, can be empty). "
            "Do not wrap the response in any other structure such as remediation_plan."
        )
    ...
    async def _analyze_with_llm(
        ...
        # Log outbound LLM request payload (truncated for readability)
        logger.info(
            "🧾 [LLM-REQUEST] Prepared prompt for violation %s: %s",
        ...
        response = await client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": self._system_message},
                {
                    "role": "user",
                    "content": (
                        "Using the incident data below, provide the remediation decision JSON as instructed.\n"
                        f"Incident data: {prompt}"
                    ),
                },
            ],
        )
        ...
        try:
            ...
            logger.info(
                "📨 [LLM-RESPONSE] Raw response for violation %s: %s",
                signal.violation.rule_id,
                (content[:1000] + ("..." if len(content) > 1000 else "")) if content else "<empty>",
            )
        ...
        logger.info(
            "🧠 [LLM-PAYLOAD] Parsed payload for violation %s: %s",
            signal.violation.rule_id,
    ...
    def _normalise_llm_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Normalise common LLM response layouts into the expected flat schema."""

        if not isinstance(payload, dict):
            return payload

        if "remediation_plan" in payload and isinstance(payload["remediation_plan"], dict):
            plan = payload["remediation_plan"]

            actions_raw = plan.get("remediation_actions")
            formatted_actions: List[str] = []
            if isinstance(actions_raw, list):
                for entry in actions_raw:
                    if isinstance(entry, dict):
                        name = entry.get("action") or entry.get("name") or entry.get("title")
                        formatted_actions.append(name or json.dumps(entry))
                    else:
                        formatted_actions.append(str(entry))

            payload = {
                "remediation_type": plan.get("remediation_type") or plan.get("decision_type") or plan.get("recommended_mode"),
                "confidence_score": plan.get("confidence_score"),
                "reasoning": plan.get("reasoning") or plan.get("summary") or plan.get("details") or "Remediation plan generated by LLM.",
                "estimated_effort": plan.get("estimated_effort"),
                "risk_if_delayed": plan.get("risk_if_delayed") or plan.get("risk_level"),
                "prerequisites": plan.get("prerequisites") or [],
                "recommended_actions": plan.get("recommended_actions") or formatted_actions,
            }

            if payload["estimated_effort"] is None and formatted_actions:
                payload["estimated_effort"] = max(30, 20 * len(formatted_actions))

            if payload["confidence_score"] is None:
                payload["confidence_score"] = 0.6

            if payload["remediation_type"] is None:
                risk_hint = str(plan.get("risk_level", "medium")).lower()
                payload["remediation_type"] = (
                    "manual_only" if risk_hint in {"high", "critical"} else "human_in_loop"
                )

            if payload["risk_if_delayed"] is None:
                payload["risk_if_delayed"] = plan.get("risk_level", "medium")

        for key in ("prerequisites", "recommended_actions"):
            ...
            if value is None:
                ...
                payload[key] = [str(entry) for entry in value.values()]
    ...
    def _parse_llm_response(self, response: str) -> Dict[str, Any]:
        ...
        if match:
            try:
                ...
                return self._normalise_llm_payload(data)
